...
function recordContext() {
  ...
  return {
    ...
    plans: C.TRIALS.map((t) => { const e = C.applyOverrides(t, state.planOverrides); return { id: t.id, name: t.name, ex: e.ex }; }),
  };
}
  ...
  try {
    switch (name) {
      ...
      case 'switch_trial': {
        ...
        if (id === 'next') { const i = C.TRIALS.findIndex((x) => x.id === trial.id); id = C.TRIALS[(i + 1) % C.TRIALS.length].id; }
        if (!C.TRIALS.find((x) => x.id === id)) return "Unknown trial. Options: anterior, posterior, sanctuary.";
        ...
        return 'Today is now ' + C.TRIALS.find((x) => x.id === id).name + '.';
      }
      case 'reopen_today': {
      ...
      case 'update_plan': {
        ...
        if (!C.TRIALS.find((x) => x.id === id)) return 'Unknown trial. Options: anterior, posterior, sanctuary.';
        const raw = Array.isArray(input.exercises) ? input.exercises : [];
        const ex = raw.slice(0, 8).map((e2) => ({
          n: String(e2.n || '').slice(0, 60), s: Math.min(6, Math.max(1, parseInt(e2.s) || 3)),
          r: String(e2.r || '8–15').slice(0, 20), w: Math.max(0, Math.min(40, Number(e2.w) || 0)),
        ...
        return C.TRIALS.find((x) => x.id === id).name + ' rewritten: ' + ex.map((e2) => e2.n + ' ' + e2.s + '×' + e2.r + (e2.w ? ' @' + e2.w : '')).join('; ') + '.';
      }
      case 'set_note': {
