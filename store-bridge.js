...
window.Ascension = {
  ...
  /** The effective program: baked defaults + structure/plan edits, with rotation position. */
  async trialsToday() {
    const c = await ctx();
    const st = c.fold();
    const list = (st.structure && st.structure.length) ? st.structure : C.TRIALS;
    const trials = list.map((t) => C.applyOverrides(t, st.planOverrides));
    const doneCount = st.sessions.filter((s) => s.completed).length;
    const today = C.dayKey();
    const todays = st.sessions.find((s) => s.completed && s.dayKey === today);
    let index = doneCount % trials.length;
    if (todays) { const i = trials.findIndex((t) => t.id === todays.trialId); if (i >= 0) index = i; }
    else if (st.trialChoices && st.trialChoices[today]) {
      const i = trials.findIndex((t) => t.id === st.trialChoices[today]); if (i >= 0) index = i;
    }
    return { trials, index, completedToday: C.completedToday(st.sessions) };
  },
